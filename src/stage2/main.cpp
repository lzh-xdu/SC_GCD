...
constexpr unsigned QUEUE_COUNT = 2;
...
void writeQueueStats(std::ostream& stats, const System& system, int depth) {
    const std::array<const char*, QUEUE_COUNT> names{"parser_to_transform", "compute_to_output"};
...
void writeHandshakeStats(std::ostream& stats, const System& system, const Options& options) {
    ...
    stats << "handshake_valid_cycles," << valid << "\nhandshake_blocked_cycles," << system.m_transform.m_blockedCycles
          ...
          << "\ncompute_units,1\ncompute_task_slots,1\ntransform_slots,2\nfifo_slots," << QUEUE_COUNT * options.m_depth
          << "\nbuffer_payload_bits,"
          << options.m_depth * (TASK_PAYLOAD_BITS + RESULT_PAYLOAD_BITS) + TRANSFORM_CAPACITY * TASK_PAYLOAD_BITS
          << '\n';
}
