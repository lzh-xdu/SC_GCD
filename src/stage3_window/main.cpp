...
/**
 * @file main.cpp
 * @brief Connect and observe the stage 3 ready-first window simulation.
 */
...
using stage3_window::Collector;
using stage3_window::Dispatcher;
using stage3_window::UNIT_COUNT;
namespace {
constexpr int MIN_ARGUMENTS = 4;
constexpr int MAX_ARGUMENTS = 11;
constexpr int WINDOW_ARGUMENT = 9;
constexpr int SEED_ARGUMENT = 10;
constexpr unsigned DEFAULT_WINDOW = 8;
constexpr std::uint32_t DEFAULT_SEED = 1;
constexpr std::uint64_t MAX_SEED = 4294967295ULL;
...
struct Options {
    unsigned m_window = DEFAULT_WINDOW;
...
System::System(sc_core::sc_module_name name, std::istream& input, std::ostream& output, TestEventLog& testEventLog,
    ...
    , m_dispatcher("dispatcher", testEventLog, options.m_window, options.m_seed)
    , m_compute0("compute0", testEventLog, 0)
    , m_compute1("compute1", testEventLog, 1)
    , m_collector("collector", testEventLog, options.m_window)
...
void System::observe() {
    while (true) {
        ...
        m_windowReserved.sample(static_cast<unsigned>(m_dispatcher.m_dispatched - m_collector.m_nextId));
        if (drained()) {
            m_finished = true;
            sc_core::sc_stop();
            return;
...
Options parseOptions(int argc, char** argv) {
    Options options;
    if (argc > WINDOW_ARGUMENT) {
        options.m_window = static_cast<unsigned>(positive(argv[WINDOW_ARGUMENT], MAX_DEPTH));
    }
    if (argc > SEED_ARGUMENT) {
        options.m_seed = static_cast<std::uint32_t>(positive(argv[SEED_ARGUMENT], MAX_SEED));
    }
...
void writeHandshakeStats(std::ostream& stats, const System& system, const Options& options) {
    ...
    stats << "handshake_valid_cycles," << valid << "\nhandshake_blocked_cycles," << system.m_transform.m_blockedCycles
          ...
          << options.m_depth * (TASK_PAYLOAD_BITS + RESULT_PAYLOAD_BITS) +
                 ...
                 TRANSFORM_CAPACITY * TASK_PAYLOAD_BITS
          << "\nwindow_blocked_cycles," << system.m_dispatcher.m_windowBlockedCycles << "\nengines_blocked_cycles,"
          << system.m_dispatcher.m_engineBlockedCycles << "\nrandom_choices," << system.m_dispatcher.m_randomChoices
          << "\nrandom_seed," << options.m_seed << "\nreorder_wait_cycles," << system.m_collector.m_orderWaitCycles
          << "\ncollector_output_blocked_cycles," << system.m_collector.m_outputBlockedCycles << '\n';
}
...
int run(int argc, char** argv) {
    ...
    stats << "window_blocked_with_ready_cycles," << system.m_dispatcher.m_windowBlockedWithReadyCycles << '\n';
...
int sc_main(int argc, char** argv) {
    if (argc < MIN_ARGUMENTS || argc > MAX_ARGUMENTS) {
        std::cerr << "Usage: stage3_window_gcd INPUT OUTPUT STATS [DEPTH=2] [EVENTS.csv|-]"
