 ...
 * 缓存：m_stage1 与 m_stage2 各保存一条，optional 表示是否有效；两侧 FIFO 在模块外。
 * 周期：k 沿接收并取绝对值 -> k+1 沿比较交换 -> k+2 沿写出，基础延迟严格为 2T。
 *        输出 FIFO 的下游最早 k+3 沿读取。D 是外部 FIFO 容量，默认 2。
 * 背压：stage2 写不出则保持；stage1 空时还能接收一条，两级满后停止接收。
 * 更新：先处理旧 stage2，再移动旧 stage1，最后接收新任务，禁止新数据同沿穿两级。
 * 旁路观察：m_testEventLog 不作为硬件资源或调度输入。
 */
SC_MODULE(Transform) {
