...
2026-09-10 晚；基线 `9dd3b1a`，测量脚本与证据为本条所在提交。按 [指标与呈现方案](archive/metrics-presentation.md) 的最小实验矩阵实际跑数，报告分为四阶段总览与对照表；场景、公式和未覆盖项沿用该设计文档。
...
- 主机测量：进程口径（含启动），每配置预热 1 次＋正式 7 次，两模型逐轮交替顺序；墙钟用 `perf_counter`，CPU 时间与峰值 RSS 取自 `wait4`。Windows 基线的模型入口口径（排除启动）数据保留在 [双模型实测](archive/model-comparison.md)，两者口径不同不可直接比较，见 §4。
...
周期契约验证：五个非空场景全部满足 **`T = B + N + 5`**（如混合 466079+10000+5=476084）。每任务服务占 `L+1` 拍（L 为取余延迟），常数 5 为启动与排空；这从数据侧印证了 Compute「完成沿不收新任务、下拍才可再接收」的时序约定。零延迟场景忙周期为 0 但吞吐 0.9998——按模型假设 `b=0` 无取余工作，不代表系统未处理任务（[设计文档](archive/metrics-presentation.md) §2 预判的情形）。混合负载 e2e 均值 331.2 拍中计算仅 46.6 拍，其余为排队（precompute 段均值 283.6 拍）。
...
读图提示：short/long 任务同质、完成有序（保序等待为 0），窗口无可缓存，两模型 T 仅差 1 拍流水常数；mixed 计算受限（利用率≈98%），保序等待与计算忙重叠、不延长 T；stage3 的每引擎 R=2 结果 FIFO 本身已是 4 项隐式乱序缓冲。窗口容量的效应见 3.2 的 W 扫描，a3_cycles 不反映 W 的作用，也不能读成缓存窗口单项的净收益（乱序输出对照未实施）。
...
- **B-011 在 Linux 复现**：混合小样本关 Trace 时事件模型比时钟快 1.29×（21.4 vs 27.5 ms），开 Trace 后反转为慢 8%（124.3 vs 114.7 ms），增幅 481% 对 317%——事件模型对跳过区间补齐逐周期控制行的实现（[B-011](archive/defects.md)）是平台无关的设计问题。
...
- delta 轮次：Linux 无 `psapi` 包装入口，本表未测；沿用 [Windows 基线](archive/model-comparison.md)（长计算 1,500,027→100,027 等），模型代码未变，该数据仍有效。

### 4.4 2026-09-11 补充：perf 指导的 Trace 优化

前述 B-011 数据保留为优化前历史。用户提出对仿真程序生成 Trace 的过程做 perf 采样；本轮采用相同 `-O3 -g -fno-omit-frame-pointer` 的新旧二进制，Linux 原生临时文件、无 profiler 干扰的串行交替测量，每配置预热一次后七次正式运行，共 84 次。口径与具体输入详见 [新报告](evidence/perf-trace/README.md)，不与旧小样本数字直接拼接。

- perf 软件 CPU 采样：前 3391、后 2973 样本，均无丢样；前版两个主要 Trace 归并排序函数 Self 合计约 13%，另有流格式化/复制热点。依据证据改成紧凑区间、小批合并、`to_chars` 和字节缓冲输出；保持完整 Trace。
- 混合一万项 Trace on：302.77 → 83.83 ms（3.61×）；同配置时钟模型 221.25 ms。峰值进程 RSS 73980 → 21052 KiB。日志同为 21635542 字节，三程序结果、全部统计及 Trace SHA256 一致，每次另经独立 GCD 验证。
- Trace off：34.92 → 35.30 ms，四分位范围重叠；零延迟 Trace 快 2.55×，慢输出 Trace 快 5.60×。B-011 已有实际改进与回归闭环，仍归类为观测实现问题，未宣称消除所有日志开销。
